...
'''
    ...
    if not cards_html:
        cards_html = '''        <div class="empty-state" style="grid-column: 1 / -1;">
          ...
          <p>AWS ニュースレポートからインフォグラフィックを生成すると、ここに表示されます。</p>
        </div>
'''
...
<head>
  ...
  <title>AWS News Infographic Gallery</title>
...
<body>
  <div class="container">
    <h1>📊 AWS News Infographic Gallery</h1>
    <p class="subtitle">AWS ニュースレポートのインフォグラフィック一覧</p>
    <div class="grid">
{cards_html}    </div>
    <footer>
      <p>Generated by AWS News Summary | {datetime.now().strftime("%Y-%m-%d %H:%M")}</p>
    </footer>
  </div>
</body>
</html>
'''
    ...
    def strip_timestamp(content: str) -> str:
        return re.sub(r"Generated by AWS News Summary \| \d{4}-\d{2}-\d{2} \d{2}:\d{2}", "", content)
                    ...
                    if isinstance(message, AssistantMessage):
                        ...
                        for block in content if isinstance(content, list) else []:
                            if isinstance(block, TextBlock):
                                ...
                                # AWS docs search
                                elif "mcp__aws-knowledge" in str(tool_name):
                                    if "search" in tool_name:
                                        search_phrase = ""
                                        if isinstance(tool_input, dict):
                                            search_phrase = tool_input.get("search_phrase", "")[:40]
                                        elapsed = logger.elapsed()
                                        print(
                                            f"\n  [Search AWS docs: {search_phrase}] ({elapsed})",
                                            end="",
                                            flush=True
                                        )
                                    elif "read" in tool_name:
                                        url = tool_input.get("url", "") if isinstance(tool_input, dict) else ""
                                        url_short = url.split("/")[-1][:30] if url else ""
                                        elapsed = logger.elapsed()
                                        print(
                                            f"\n  [Read AWS docs: {url_short}] ({elapsed})",
                                            end="",
                                            flush=True
                                        )
                                    else:
