...
| AWS ダーク | #[[file:themes/aws-dark.md]] | AWS プレゼンテーション風ダークテーマ | 「AWS」「ダーク」「Squid Ink」「技術資料」「プレゼン風」 |
...
- **システムアーキテクチャ**: AWS サービス間の連携、データフロー、ネットワーク構成
...
- **階層構造**: サブグラフやグループ化が必要な構成 (リージョン、アカウント、VPC など)
...
- **マルチアカウント構成**: クロスアカウントアクセス、組織構造
- **ネットワーク構成**: VPC ピアリング、Transit Gateway、複数サブネット
...
<div class="flow-diagram">
    ...
    <span class="flow-arrow">→</span>
    <div class="flow-step">⚡ Lambda<br/>実行</div>
    <span class="flow-arrow">→</span>
...
<div class="architecture-grid">
    <div class="arch-component primary">🌐 CloudFront</div>
    <div class="arch-arrow">↓</div>
    <div class="arch-component">🔌 API Gateway</div>
    <div class="arch-arrow">↓</div>
    <div class="arch-component">⚡ Lambda</div>
    <div class="arch-arrow-group">
        <span>↙</span><span>↘</span>
    </div>
    <div class="arch-row">
        <div class="arch-component storage">🗄️ DynamoDB</div>
        <div class="arch-component storage">🪣 S3</div>
    </div>
</div>
```
...
- **設定例**: CloudFormation、Terraform、AWS CLI コマンド
- **実装パターン**: Lambda ハンドラー、イベント処理、エラーハンドリング
...
**注意**: ダークテーマ (AWS Dark, AWS News など) では `atom-one-dark` を、ライトテーマ (AWS Light など) では `atom-one-light` を使用すること。
...
<div class="code-block">
    <div class="code-header">
        ...
        <span class="code-title">Lambda ハンドラー例</span>
    </div>
...
- **例**: `20260108-aws-architecture-overview.html`, `20260108-meeting-summary.html`
- 日付は生成日を使用
