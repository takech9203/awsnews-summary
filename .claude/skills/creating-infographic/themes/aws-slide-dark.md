# AWS スライドダークテーマ（複数ページ PDF 対応）

AWS 公式プレゼンテーションスタイル (re:Invent、AWS Summit、ウェビナー等) に準拠したダークテーマのスライド形式。PDF 印刷時に複数ページとして出力可能。
...
5. **タイトルは白色**: スライドタイトルは白色 (#FFFFFF) で視認性を確保
5. **左揃え基本**: タイトルスライドを含むすべてのスライドで左揃え
7. **絵文字を活用**: タイトル、カード見出し、リストアイコンに積極的に使用。ただし文章内の過剰な使用は避ける
8. **コントラスト確保**: ダーク背景に対して十分な明るさのテキスト色を使用
...
7. **ノートボックス**: 補足情報は `.note-box`、AI 関連は `.ai-box`、警告は `.warning-box`
...
| 補足・注意 | ノートボックス (`.note-box`, `.warning-box`, `.ai-box`) |
...
<palette name="aws-slide-dark">
  ...
  <color name='Smile Orange' rgb='FFAD33' r='255' g='173' b='51' description='セカンダリアクセント' />
  
  <!-- Text Colors -->
  <color name='White' rgb='FFFFFF' r='255' g='255' b='255' description='タイトル、見出し' />
  <color name='Light Gray' rgb='D5DBDB' r='213' g='219' b='219' description='本文テキスト' />
  <color name='Medium Gray' rgb='879196' r='135' g='145' b='150' description='サブテキスト、フッター' />
  
  <!-- Supporting Colors -->
  <color name='Dark Squid Ink' rgb='1A242F' r='26' g='36' b='47' description='カード背景' />
  <color name='Darker Squid' rgb='131920' r='19' g='25' b='32' description='より暗い背景' />
  ...
  <!-- Tint Colors (ダーク背景用の半透明色) -->
  <color name='Orange Tint' rgb='rgba(255,153,0,0.15)' description='オレンジ系ハイライト背景' />
  <color name='Teal Tint' rgb='rgba(0,161,201,0.15)' description='ティール系ノート背景' />
  <color name='Purple Tint' rgb='rgba(140,79,255,0.15)' description='AI/ML 関連背景' />
</palette>
```
...
- **ダーク背景**: Squid Ink (#232F3E) を基調とした落ち着いた配色

### タイポグラフィ (スライド向け大きめサイズ)

| 要素 | フォントサイズ | ウェイト | 色 |
|------|---------------|---------|-----|
| タイトルスライド見出し | 56px | 700 (Bold) | #FFFFFF |
| タイトルスライドサブタイトル | 28px | 400 | #D5DBDB |
| スライドタイトル | 40px | 600 (SemiBold) | #FFFFFF |
| 本文・箇条書き | 26px | 400 | #D5DBDB |
| 強調テキスト | 26px | 600 | #FFFFFF |
| フッター・ページ番号 | 16px | 400 | #879196 |
...
.slide.title-slide {
  ...
  text-align: left;
  padding-top: 56px;
  background: linear-gradient(135deg, var(--aws-squid-ink) 0%, var(--aws-dark-squid) 100%);
}
...
<ul class="content-list">
  <li style="flex-direction: column; align-items: flex-start; gap: 4px;">
    ...
    <div style="margin-left: 28px; font-size: 16px; color: var(--aws-light-gray);">
      説明テキスト
    </div>
  </li>
</ul>
...
スライド下部に大きな余白が生じる場合は、以下の方法で有効活用すること。

- カード内の情報を充実させる (概要文 + 箇条書きで詳細を追加)
- ノートボックスでサマリーや補足情報を追加
- フォントサイズを調整してコンテンツを拡大

余白を埋めるために無関係な情報を追加しないこと。
...
| カード内リスト項目 | 18px | 20px |
| ノートボックス (.note-box, .ai-box) | 18px | 20px |
| アジェンダリスト項目 | 20px | 22px |
...
| 2 | 下部に空白 | `.note-box` / `.ai-box` でサマリーや補足を追加 |
...
```css
.slide-content {
  justify-content: space-between;  /* flex-start ではなく space-between */
}
...
|------|-----------|-----------|
| タイトル | 32px | 40-56px |
| 本文 (.slide-content p) | 20px | 22px |
| 箇条書き (.content-list li) | 20px | 22px |
| アジェンダ項目 | 20px | 22px |
| カードタイトル | 20px | 24px |
| カードコンテンツ | 18px | 20px |
| フロー図ステップ | 16px | 18px |
...
- **白 (#FFFFFF)**: スライドタイトル、タイトルスライド見出し、強調テキスト
- **オレンジ (#FF9900)**: 左サイドバー、アクセント、重要なポイント
- **ライトグレー (#D5DBDB)**: 本文テキスト
- **ダーク背景 (#1A242F)**: カード背景
